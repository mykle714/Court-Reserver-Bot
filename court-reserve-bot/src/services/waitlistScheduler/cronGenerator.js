...
/**
 * Generates and manages cron jobs for waitlist targets
 */
class CronGenerator {
  constructor() {
    this.jobs = new Map(); // Map of targetId -> interval reference
    this.checkIntervalSeconds = config.scheduler.checkIntervalSeconds;
    this.advanceCheckDays = config.scheduler.advanceCheckDays;
  }
   ...
   * @param {Object} target - Waitlist target configuration
   * @param {Function} checkCallback - Function to call when checking
   * @returns {Object|null} Created job or null if not scheduled
   */
  createJob(target, checkCallback) {
    ...
    // Create interval-based job (more precise than cron for seconds)
    const intervalMs = this.getCheckIntervalMs();
    const intervalId = setInterval(async () => {
      try {
        logger.debug(`Running check for target ${id}`);
        await checkCallback(target);
      } catch (error) {
